...
export async function exportBackup(): Promise<void> {
  ...
  const file = new ExpoFile(Paths.document, 'habitquest-backup.json');
  file.write(json);
...
export async function importBackup(onSuccess: () => void): Promise<void> {
  ...
  try {
    const result = await ExpoFile.pickFileAsync('application/json');
    if (Array.isArray(result)) {
      if (result.length === 0) return;
      picked = result[0] as PickedFile;
    } else {
      picked = result as PickedFile;
    }
  } catch {
    return;
  }
  ...
  Alert.alert(
    ...
    [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: () => {
          if (!runMigrations()) {
            Alert.alert('Erro', 'Nao foi possivel preparar o banco antes da restauracao.');
            return;
          }
          ...
          try {
            ...
            Alert.alert('Erro', 'Falha ao restaurar backup. Nenhuma alteracao foi aplicada.');
          }
        },
      },
    ]
